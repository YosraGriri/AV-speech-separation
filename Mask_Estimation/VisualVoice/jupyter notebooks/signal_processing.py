...
def istft(stft_signal, size, shift=1024,
          window=signal.blackman, fading=True, window_length=None):
    """
    ...
    print(size // 2) 
    assert stft_signal.shape[1] == size // 2 
    ...
    print(f'Shape of window is: {window.shape}')
    ...
    for j, i in enumerate(range(0, len(time_signal) - size + shift, shift)):
        print(f'Shape of window is: {window.shape}')
        print(f'Shape of stft_signal is: {irfft(stft_signal[j]).shape}')
