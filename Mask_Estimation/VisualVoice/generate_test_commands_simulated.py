...
def select_random_files_original(base_dir, n_pairs=1, n_mics=3):
    ...
    #audio_base = 'E:/AV-speech-separation/data/VoxCeleb2/raw_audio_simulated'
    #video_base = 'E:/AV-speech-separation/data/VoxCeleb2/mp4'
    mouth_roi_base = '../../data/test/mouth_roi'
    audio_base = '../../data/test/raw_audio/simulated'
    ...
    for _ in range(n_pairs):
        ...
        for index, random_id in enumerate(selected_random_ids):
            ...
            audio_path_base = os.path.join(audio_speaker_path, f"{file_id}.wav_mic")
            ...
            # Store paths and base audio path for microphone iteration
            segment_data.append({
                'index': index,
                'audio_path_base': audio_path_base,
                'mouth_roi_file_path': mouth_roi_file_path,
        ...
        for mic_index in range(n_mics):
            for data in segment_data:
                index = data['index']
                audio_path = f"{data['audio_path_base']}{mic_index}_voice{index}.wav"
                ...
                if index == 0:
                    ...
                    mouth_roi1_path = data['mouth_roi_file_path']
                    video1_path = data['video_path']
                else:
                    audio2_path = audio_path
                    mouth_roi2_path = data['mouth_roi_file_path']
                    video2_path = data['video_path']
...
if __name__ == "__main__":
    ...
    n_mics = 4  # Example: loop over microphones 0 and 1

    # Select random files and generate pairs
    selections = select_random_files_original(base_dir, n_pairs=2, n_mics=n_mics)
    from pprint import pprint
