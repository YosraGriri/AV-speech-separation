...
def get_separated_audio(
    ...
    preds_wav = utils.istft_reconstruction_from_complex(pred_spec_real, pred_spec_imag, hop_length=opt.hop_size,
...
def main():
    ...
    for speaker_index in range(opt.number_of_speakers):
        ...
        while sliding_window_start + samples_per_window < audio_length:
            ...
            reconstructed_signal = get_separated_audio(net_lipreading, net_facial_attributes, net_unet, audio_spec,
                                                       segment_mouthroi, frames, opt)
            reconstructed_signal = reconstructed_signal * normalizer
            sep_audio[sliding_window_start:sliding_window_end] = sep_audio[
        ...
        sf.write(os.path.join(opt.output_dir_root, 'speaker' + str(speaker_index + 1) + '.wav'), avged_sep_audio,
